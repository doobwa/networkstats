...
public:
  ...
  void initializeNetwork(IntegerVector tailsv, IntegerVector headsv, int nedges, 
    ...
    Vertex nnodes, bip;
    Network nw;
    nnodes = (Vertex)dn; /* coerce double *dn to type Vertex */
    ...
    nw = NetworkInitialize(tails, heads, nedges, nnodes, directed_flag, bip, 0);
    this->nw = nw;
  }
  void initializeModel(char **funnames, char **sonames, double *inputs, int *nterms) {
    Model *m;
    m = ModelInitialize(*funnames, *sonames, inputs, *nterms);
    this->m = *m;
  }
  int toggleEdge(int tail, int head) {
  }
  int toggleEdgelist(int tails, int heads) {
  }
...
RCPP_MODULE(change_score_network2){
	...
	class_<ChangeScoreNetwork2>( "ChangeScoreNetwork2" )
          ...
          .method("initializeNetwork", &ChangeScoreNetwork2::initializeNetwork, 
                  "set the message" )
          .method("getNumEdges", &ChangeScoreNetwork2::getNumEdgesNetwork, 
                  "set the message" )
